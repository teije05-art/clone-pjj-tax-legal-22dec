# Track Plan: Refactor `searcher.py` to use Gemini and add fallback
...
- [ ] Task: Refactor `searcher.py`
    - [ ] Sub-task: Update the `extract_semantics` method (or similar) to use the `google-genai` client with the `gemini-3-flash` model.
    - [ ] Sub-task: Remove all references and dependencies related to Llama and Fireworks AI.
    - [ ] Sub-task: Implement a keyword-based search on filenames in `data/tax_legal` as a fallback if the semantic search yields no results.
